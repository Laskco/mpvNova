...
5. Build app via `./buildall.sh --clean -n`
   ...
   Make sure release signing is configured first.
   Use either a local `keystore.properties` file based on `keystore.properties.example`
   or the `MPVNOVA_STORE_FILE`, `MPVNOVA_STORE_PASSWORD`, `MPVNOVA_KEY_ALIAS`,
   and `MPVNOVA_KEY_PASSWORD` environment variables.
